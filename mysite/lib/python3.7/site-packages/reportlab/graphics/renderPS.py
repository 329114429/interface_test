...
''' % (self.width,self.height))
    ...
    def _drawImageLevel1(self, image, x1, y1, width=None, height=None):
        ...
        # now I need to tell postscript how big images is

        # "images operators assume that they receive sample data from
        ...
        #
        # The ImageMatrix maps unit squre of user space to boundary of the source images
        #

        # The CurrentTransformationMatrix (CTM) maps the unit square of
        # user space to the rect...on the page that is to receive the
        # images. A common ImageMatrix is [width 0 0 -height 0 height]
        # (for a left to right, top to bottom images )

        # first let's map the user coordinates start at offset x1,y1 on page

        self.code.extend([
            'gsave',
            '%s %s translate' % (x1,y1), # need to start are lower left of images
            '%s %s scale' % (width,height),
            '/scanline %d 3 mul string def' % imgwidth  # scanline by multiples of images width
            ])
        ...
        while dataline != "":
            ...
            dataline= outstream.read(78)
        self.code_append('% end of images data') # for clarity
    ...
    def _drawImageLevel2(self, image, x1,y1, width=None,height=None): # Postscript Level2 version
        ...
        ### what sort of images are we to draw
        if image.mode=='L' :
...
<<
/ImageType 1
/Width %d /Height %d  %% dimensions of source images
/BitsPerComponent %d""" % (imwidth, imheight, imBitsPerComponent) )
        ...
        self.code.extend([  '/ImageMatrix [%s 0 0 %s 0 %s]' % (imwidth, -imheight, imheight),
                            ...
                            '>> % End images dictionary',
                            'images'])
        # after images operator just need to dump images dat to file as hexstring
        rawimage = (myimage.tobytes if hasattr(myimage,'tobytes') else myimage.tostring)()
        hex_encoded = self._AsciiHexEncode(rawimage)
        ...
        while dataline != "":
            self.code_append(dataline)
            dataline= outstream.read(78)
        self.code_append('> % end of images data') # > is EOD for hex encoded filterfor clarity
        self.code_append('grestore') # return coordinates to normal
