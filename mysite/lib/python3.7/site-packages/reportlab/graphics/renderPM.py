...
class _PMRenderer(Renderer):
    """This draws onto a pix map images. It needs to be a class
    rather than a function, as some images-specific state tracking is
...
class PMCanvas:
    def __init__(self,w,h,dpi=72,bg=0xffffff,configPIL=None):
        '''configPIL dict is passed to images save method'''
        scale = dpi/72.0
        w = int(w*scale+0.5)
        h = int(h*scale+0.5)
    ...
    def saveToFile(self,fn,fmt=None):
        ...
        if fmt in ('GIF',):
            ...
            raise RenderPMError("Unknown images kind %s" % fmt)
        if fmt=='TIFF':
