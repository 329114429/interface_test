...
def makeA85Image(filename,IMG=None, detectJpeg=False):
    ...
    code = []
    append = code.append
    # this describes what is in the images itself
    append('BI')
    append('/W %s /H %s /BPC 8 /CS /%s /F [/A85 /Fl]' % (imgwidth, imgheight,_mode2cs[img.mode]))
    append('ID')
    #use a flate filter and Ascii Base 85
    assert len(raw) == imgwidth * imgheight*_mode2bpp[img.mode], "Wrong amount of data for images"
...
def cacheImageFile(filename, returnInMemory=0, IMG=None):
    "Processes images as if for encoding, saves to a file with .a85 extension."
    ...
    if filename==cachedname:
        if cachedImageExists(filename):
            ...
            raise IOError('No such cached images %s' % filename)
    else:
        if rl_config.useA85:
        ...
        if rl_config.verbose:
            print('cached images as %s' % cachedname)


def preProcessImages(spec):
    """Preprocesses one or more images files.

    Accepts either a filespec ('C:\\mydir\\*.jpg') or a list
    of images filenames, crunches them all to save time.  Run this
    to save huge amounts of time when repeatedly building images
    documents."""
...
def cachedImageExists(filename):
    """Determines if a cached images already exists for a given file.

    Determines if a cached images exists which has the same name
