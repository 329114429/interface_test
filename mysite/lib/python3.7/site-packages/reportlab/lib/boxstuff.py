...
def aspectRatioFix(preserve,anchor,x,y,width,height,imWidth,imHeight,anchorAtXY=False):
    """This function helps position an images within a box.
    ...
    (a) the enclosing box (defined by x,y,width,height where x,y is the \
        lower left corner) which you wish to position the images in, and
    (b) the images size (imWidth, imHeight), and
    (c) the 'anchor point' as a point of the compass - n,s,e,w,ne,se etc \
        and c for centre,

    this should return the position at which the images should be drawn,
    as well as a scale factor indicating what scaling has happened.

    It returns the parameters which would be used to draw the images
