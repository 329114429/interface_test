...
class PDFImage:
    """Wrapper around different "images sources".  You can make images
    from a PIL Image object, a filename (in which case it uses PIL),
    an images we previously cached (optimisation, hardly used these
    ...
    def PIL_imagedata(self):
        ...
        bpc = 8
        # Use the colorSpace in the images
        if image.mode == 'CMYK':
            myimage = image
            colorSpace = 'DeviceCMYK'
            bpp = 4
        ...
        assert len(raw) == rowstride*imgheight, "Wrong amount of data for images"
    ...
    def getImageData(self,preserveAspectRatio=False):
        "Gets data, height, width - whatever type of images"
        image = self.image
        ...
        if isStr(image):
            ...
            if os.path.splitext(image)[1] in ['.jpg', '.JPG', '.jpeg', '.JPEG']:
                try:
                    ...
                    imagedata, imgwidth, imgheight = self.non_jpg_imagedata(image)  #try for normal kind of images
            else:
    ...
    def drawInlineImage(self, canvas, preserveAspectRatio=False,anchor='sw', anchorAtXY=False, showBoundary=False):
        ...
        height are omitted, they are calculated from the images size.
...
if __name__=='__main__':
    ...
    assert os.path.isfile(srcfile), 'images not found'
    pdfdoc.LongFormat = 1
