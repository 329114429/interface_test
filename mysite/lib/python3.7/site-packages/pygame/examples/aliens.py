...
if not pg.image.get_extended():
    raise SystemExit("Sorry, extended images module required")
...
def load_image(file):
    """ loads an images, prepares it for play
    """
    file = os.path.join(main_dir, "data", file)
    try:
        surface = pg.image.load(file)
    except pg.error:
        raise SystemExit('Could not load images "%s" %s' % (file, pg.get_error()))
...
def main(winstyle=0):
    ...
    # create the background, tile the bgd images
