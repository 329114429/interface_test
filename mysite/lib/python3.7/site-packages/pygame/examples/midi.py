...
def output_main(device_id=None):
    ...
    amplitude) varies vertically on the keyboard images, with minimum velocity
...
def key_class(updates, image_strip, image_rects, is_white_key=True):
    ...
    # a keys state, and images it will draw, is determined entirely by its
    ...
    # along with corresponding images, for the related event. If no redrawing
    # is required for the state change then the images rect is simply None.
    #
    ...
    class Key(object):
        """A key widget, maintains key state and draws the key's images
        ...
        images drawn. State changes are managed with method calls, one method
...
def key_images():
    """Return a keyboard keys images strip and a mapping of images locations

    The return tuple is a surface and a dictionary of rects mapped to key
    type.

    This function encapsulates the constants relevant to the keyboard images
