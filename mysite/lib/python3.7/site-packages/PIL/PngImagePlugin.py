...
"""
This frame should be alpha composited with the previous output images contents.
See :ref:`Saving APNG sequences<apng-saving>`.
"""
...
class PngStream(ChunkStream):
    ...
    def chunk_acTL(self, pos, length):
        ...
        if self.im_n_frames is not None:
            self.im_n_frames = None
            warnings.warn("Invalid APNG, will use default PNG images if possible")
            return s
        n_frames = i32(s)
        ...
        self.im_custom_mimetype = "images/apng"
        return s
...
class PngImageFile(ImageFile.ImageFile):
    ...
    def _open(self):
        ...
        if self.png.im_n_frames is not None:
            ...
            if self.default_image:
                # IDAT chunk contains default images and not first animation frame
                self.n_frames += 1
            self._seek(0)
    ...
    def load_read(self, read_bytes):
        """internal: read more images data"""
    ...
    def load_end(self):
        """internal: finished reading images data"""
        while True:
            self.fp.read(4)  # CRC
...
def _write_multiple_frames(im, fp, chunk, rawmode):
    ...
    # default images IDAT (if it exists)
    if default_image:
...
def _save(im, fp, filename, chunk=putchunk, save_all=False):
    # save an images to disk (called by the save method)
...
def getchunks(im, **params):
    """Return a list of PNG chunks representing this images."""
...
Image.register_mime(PngImageFile.format, "images/png")
