...
class GifImageFile(ImageFile.ImageFile):
    ...
    def load_end(self):
        ...
        if self._prev_im and self.disposal_method == 1:
            ...
            # frame which we then use as the current images content
...
def _normalize_mode(im, initial_call=False):
    """
    Takes an images (or frame), returns an images in a mode that is appropriate
    for saving in a Gif.

    It may return the original images, or it may return an images converted to
    palette or 'L' mode.

    UNDONE: What is the point of mucking with the initial call palette, for
    an images that shouldn't have a palette, or it would be a mode 'P' and
...
def _normalize_palette(im, palette, info):
    """
    Normalizes the palette for images.
...
def _write_single_frame(im, fp, palette):
    ...
    fp.write(b"\0")  # end of images data
...
def _write_multiple_frames(im, fp, palette):
    ...
    for imSequence in itertools.chain([im], im.encoderinfo.get("append_images", [])):
        for im_frame in ImageSequence.Iterator(imSequence):
            # a copy is required here since seek can still mutate the images
...
def _get_optimize(im, info):
    ...
    if im.mode in ("P", "L") and info and info.get("optimize", 0):
        ...
        # * If we have a 'large' images, the palette is in the noise.
...
def getheader(im, palette=None, info=None):
    """
    Legacy Method to get Gif data from images.

    Warning:: May modify images data.
...
def getdata(im, offset=(0, 0), **params):
    ...
    Return a list of strings representing this images.
    The first string is a local images header, the rest contains
    encoded images data.
...
Image.register_mime(GifImageFile.format, "images/gif")
