...
# TIFF file handling
#
# TIFF is a flexible, if somewhat aged, images file format originally
...
class ImageFileDirectory_v1(ImageFileDirectory_v2):
    ...
    Also contains a dictionary of tag types as read from the tiff images file,
...
class TiffImageFile(ImageFile.ImageFile):
    ...
    def _open(self):
        """Open the first images in a TIFF file"""
    ...
    def seek(self, frame):
        """Select a given frame as current images"""
        if not self._seek_check(frame):
            return
        self._seek(frame)
        # Create a new core images object on second and
        # subsequent frames in the images. Image may be
        # different size/mode.
    ...
    def _load_libtiff(self):
        ...
        try:
            ...
            raise OSError("Couldn't set the images") from e
    ...
    def _setup(self):
        """Setup this images object based on current tags"""
        ...
        if len(sampleFormat) > 1 and max(sampleFormat) == min(sampleFormat) == 1:
            # SAMPLEFORMAT is properly per band, so an RGB images will
        ...
        if self.use_load_libtiff:
            ...
            # function.
            #
            # Setup the one tile for the whole images, then
            # use the _load_libtiff function.

            # libtiff handles the fillmode for us, so 1;IR should
            # actually be 1;I. Including the R double reverses the
            # bits, so stripes of the images are reversed.  See
            ...
            # we're expecting images byte order. So, if the rawmode
            # contains I;16, we need to convert from native to images
            # byte order.
            if rawmode == "I;16":
                rawmode = "I;16N"
            if ";16B" in rawmode:
...
# little endian is default except for images modes with
...
def _save(im, fp, filename):
    ...
    # inspired by images-sig posting from Kevin Cazabon, kcazabon@home.com
    if hasattr(im, "tag_v2"):
        # preserve tags from original TIFF images file
        for key in (
            RESOLUTION_UNIT,
            X_RESOLUTION,
            Y_RESOLUTION,
    ...
    if libtiff:
        ...
        types = {}
        # SAMPLEFORMAT is determined by the images format and should not be copied
        # from legacy_ifd.
        ...
        # we're storing images byte order. So, if the rawmode
        # contains I;16, we need to convert from native to images
        # byte order.
...
Image.register_mime(TiffImageFile.format, "images/tiff")
