...
class Captcha:
    ...
    def __init__(self, width, height, fonts=None, color=None):
        self._image = None
        self._fonts = fonts if fonts else [os.path.join(os.path.dirname(__file__), 'fonts', font) for font in
                                           ['msyh.tff', 'msyhbd.tff', ]]
    ...
    def _noise(self, number=50, level=2, color=None):
        ...
        for i in range(number):
            ...
            y = int(random.randint(dy, height))
            draw.line(((x, y), (x + level, y)), fill=color if color else self._color, width=level)

    def _text(self, captcha_text, fonts, font_size=None, drawings=None, squeeze_factor=0.75, color=None):
        """绘制文本"""
        ...
        for c in captcha_text:
            ...
            for drawing in drawings:
                ...
                char_name = d(char_image)
            char_images.append(char_image)
        width, height = self._image.size
        offset = int(
            (width - sum(int(i.size[0] * squeeze_factor) for i in char_images[:-1]) - char_images[-1].size[0]) / 2)
    ...
    def _warp(image, dx_factor=0.3, dy_factor=0.3):
        ...
        return warp_image.transform((width, height), Image.QUAD,
                                    (x1, y1, -x1, height2 - y2, width2 + x2, height + y2, width - x2, -y1))
    ...
    def generate(self, captcha_text='', fmt='PNG'):
        ...
        self._background()
        self._text(captcha_text, self._fonts, drawings=['_warp', '_rotate', '_offset'])
        self._curve()
        self._noise()
        image_bytes = BytesIO()
